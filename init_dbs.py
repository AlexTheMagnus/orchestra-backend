...
def create_favorite_table():
    db.Table('favorite', metadata,
							db.Column('user_id', db.String(36), db.ForeignKey("user.user_id"), nullable=False, primary_key=True),
...
def create_like_table():
    db.Table('like', metadata,
             	db.Column('user_id', db.String(36), db.ForeignKey("user.user_id"), nullable=False, primary_key=True),
             	db.Column('soundtrack_id', db.String(36), db.ForeignKey("soundtrack.soundtrack_id"), nullable=False, primary_key=True)
						)

def create_chapter_table():
    db.Table('chapter', metadata,
							db.Column('chapter_id', db.String(36), nullable=False, primary_key=True),
							db.Column('soundtrack_id', db.String(36), db.ForeignKey("soundtrack.soundtrack_id"), nullable=False),
